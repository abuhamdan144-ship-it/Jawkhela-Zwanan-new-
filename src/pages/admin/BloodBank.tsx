  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex gap-4 items-center flex-wrap">
        <select 
          className="bg-[#1a2a4a] border border-[#263c69] text-slate-300 rounded-lg p-2 text-sm focus:outline-none focus:border-[#d4af37]"
          value={filter}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
        {filteredMembers.map(m => {
          ...
          return (
            <div key={m.id} className="bg-[#1a2a4a] border border-[#263c69] rounded-xl p-5 flex flex-col relative overflow-hidden shadow-lg">
              {m.bloodGroup && (
              ...
              <div className="flex items-center gap-4 mb-4">
                {m.photoURL ? (
                  <img src={m.photoURL} alt={m.name} className="w-16 h-16 rounded-full object-cover border-2 border-[#33508a]" />
                ) : (
                  <div className="w-16 h-16 rounded-full bg-[#263c69] flex items-center justify-center text-slate-500 border-2 border-[#33508a]">
                    {m.name?.charAt(0) || '?'}
                  </div>
                )}
                <div>
                  ...
                  <div className="flex gap-2 mt-1">
                    ...
                    <span className={`text-[10px] uppercase font-bold tracking-wider px-2 py-0.5 rounded-full ${m.role === 'admin' ? 'bg-purple-500/20 text-purple-400' : 'bg-blue-500/20 text-[#f3d472]'}`}>
                      {m.role || 'member'}
                    </span>
                  </div>
                </div>
              ...
              <div className="space-y-2 text-sm text-slate-400 flex-grow mb-4 bg-[#0b1221] p-3 rounded-lg border border-[#263c69]">
              ...
              <div className="flex gap-2 mt-auto pt-4 border-t border-[#263c69]">
                {m.status !== 'approved' && (
        ...
        {filteredMembers.length === 0 && (
          <div className="col-span-full p-8 text-center text-slate-500 border border-[#263c69] rounded-xl bg-[#1a2a4a]">
...
function MemberEditor({ member, onSave, onCancel, bloodGroups }: any) {
  ...
  return (
    <div className="bg-[#1a2a4a] border border-[#d4af37] rounded-xl p-5 flex flex-col relative shadow-[0_0_15px_rgba(59,130,246,0.1)]">
      <div className="flex justify-between items-center mb-4 pb-2 border-b border-[#263c69]">
      ...
      <div className="space-y-4 flex-grow mb-6">
        <div>
          ...
          <div className="flex items-center gap-3">
            {formData.photoURL ? (
              <img src={formData.photoURL} alt="Preview" className="w-12 h-12 rounded-full object-cover border border-[#33508a]" />
            ) : (
              <div className="w-12 h-12 rounded-full bg-[#263c69] border border-[#33508a] flex items-center justify-center text-slate-500 text-xs">None</div>
            )}
        ...
        <div>
          ...
          <input type="text" value={formData.name || ''} onChange={e => setFormData({...formData, name: e.target.value})} className="w-full px-3 py-2 bg-[#0b1221] border border-[#263c69] rounded-lg text-sm text-white focus:border-[#d4af37] outline-none" />
        </div>
        
        <div className="grid grid-cols-2 gap-3">
          <div>
            <label className="block text-xs font-semibold text-slate-400 uppercase mb-1">Email</label>
            <input type="email" value={formData.email || ''} onChange={e => setFormData({...formData, email: e.target.value})} className="w-full px-3 py-2 bg-[#0b1221] border border-[#263c69] rounded-lg text-sm text-white focus:border-[#d4af37] outline-none" />
          </div>
          <div>
            <label className="block text-xs font-semibold text-slate-400 uppercase mb-1">Phone</label>
            <input type="text" value={formData.phone || ''} onChange={e => setFormData({...formData, phone: e.target.value})} className="w-full px-3 py-2 bg-[#0b1221] border border-[#263c69] rounded-lg text-sm text-white focus:border-[#d4af37] outline-none" />
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 gap-3">
          <div>
            ...
            <input type="text" value={formData.idCard || ''} onChange={e => setFormData({...formData, idCard: e.target.value})} className="w-full px-3 py-2 bg-[#0b1221] border border-[#263c69] rounded-lg text-sm text-white focus:border-[#d4af37] outline-none" />
          </div>
          <div>
            <label className="block text-xs font-semibold text-slate-400 uppercase mb-1">Blood Group</label>
            <select value={formData.bloodGroup || ''} onChange={e => setFormData({...formData, bloodGroup: e.target.value})} className="w-full px-3 py-2 bg-[#0b1221] border border-[#263c69] rounded-lg text-sm text-white focus:border-[#d4af37] outline-none">
      ...
      <div className="flex gap-2 mt-auto pt-4 border-t border-[#263c69]">
        ...
        <Button onClick={() => onSave(member.id, formData)} size="sm" className="flex-1 bg-[#d4af37] text-[#0b1221] font-bold hover:bg-[#f3d472] border-0">
